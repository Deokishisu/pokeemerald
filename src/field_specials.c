        ...
        if (obj != EVENT_OBJECTS_COUNT)
        {
            ...
            switch (graphicsId)
            {
                case EVENT_OBJ_GFX_LINK_RS_BRENDAN:
                    LoadPalette(gEventObjectPalette0, 0x100 + (adjustedPaletteNum << 4), 0x20);
                    break;
                case EVENT_OBJ_GFX_LINK_RS_MAY:
                    LoadPalette(gEventObjectPalette0, 0x100 + (adjustedPaletteNum << 4), 0x20);
                    break;
                case EVENT_OBJ_GFX_ZRIVAL_BRENDAN_NORMAL:
                    LoadPalette(gEventObjectPalette2, 0x100 + (adjustedPaletteNum << 4), 0x20);
                    break;
...
{
    u8 obj = SpawnSpecialEventObjectParameterized(EVENT_OBJ_GFX_BLAINE, 8, EVENT_OBJ_ID_CAMERA, gSaveBlock1Ptr->pos.x + 7, gSaveBlock1Ptr->pos.y + 7, 3);
...
{
    if (!FlagGet(FLAG_RUSTURF_TUNNEL_OPENED) && gSaveBlock1Ptr->location.mapGroup == MAP_GROUP(NONE) && gSaveBlock1Ptr->location.mapNum == MAP_NUM(NONE))
    {
...
u16 sub_81399F4(void)
{
    ...
    if (gSaveBlock1Ptr->dynamicWarp.mapGroup == MAP_GROUP(GOLDENROD_CITY_DEPARTMENT_STORE_1F))
    {
        switch (gSaveBlock1Ptr->dynamicWarp.mapNum)
        {
            case MAP_NUM(GOLDENROD_CITY_DEPARTMENT_STORE_5F):
