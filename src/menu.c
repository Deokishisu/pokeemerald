...
u8 sub_81979C4(u8 a1)
{
    if (sStartMenuWindowId == 0xFF)
        sStartMenuWindowId = sub_8198AA4(0, 16, 2, 8, (a1 * 2), 0xF, 0x139); //controls start menu positioning. Seems to be identical to WindowTemplate struct
...
void RedrawMenuCursor(u8 oldPos, u8 newPos)
{
    ...
    FillWindowPixelRect(sMenu.windowId, 0x11, sMenu.left, (sMenu.optionHeight * oldPos + sMenu.top) -1, width, height);
    AddTextPrinterParameterized(sMenu.windowId, sMenu.fontId, gText_SelectorArrow3, sMenu.left, (sMenu.optionHeight * newPos + sMenu.top) -1, 0, 0);
}
